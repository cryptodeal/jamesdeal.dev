<script>
	...
	const { data, x, xScale, y, yScale, width, height } = getContext('LayerCake');
	const heatScale = ['#5458A2', '#6689BB', '#FADC97', '#F08460', '#B02B48'];
	...
	const calcHexRadius = (radiusScale, bin) => {
		...
		if (bin.length > radiusScale[0] && bin.length <= radiusScale[1])
			return hexbinConfig.hexagonRadiusSizes[1];
	...
	const calcHeatscale = (bin) => {
		...
		if (efficiency <= 0.2) return heatScale[0];
		if (efficiency > 0.2 && efficiency <= 0.4) return heatScale[1];
		if (efficiency > 0.4 && efficiency <= 0.6) return heatScale[2];
		if (efficiency > 0.6 && efficiency <= 0.8) return heatScale[3];
		if (efficiency > 0.8) return heatScale[4];
	};

	$: hexbin = d3Hexbin.hexbin().size([$width, $height]).radius(hexbinConfig.hexagonRadius);
...
<g>
	{#each allHexbins as bin}
		<path
			d="M{bin.x},{bin.y}{hexbin.hexagon(calcHexRadius(radiusScale, bin))}"
			fill={calcHeatscale(bin)}
			fill-opacity=".7"
		/>
	{/each}
</g>
